...
namespace OracuMundial2026.Web.Services
{
    public class RankingRefreshService
    {
        ...
        private readonly OracuMundial2026Config _config;

        public RankingRefreshService(HttpClient http, IWebHostEnvironment environment, IOptions<OracuMundial2026Config> options)
        {
            _http = http;
        ...
        public async Task<RankingRefreshReport> RefreshAsync(DateOnly? today = null, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                await WriteAtomicAsync(Path.Combine(dataDirectory, OracuMundial2026DataFiles.FifaRankingsCsv), csv, ct);
                fifaRows = rows.Count;
                fifaDate = DateOnly.ParseExact(rows[0].RankingDate, "yyyy-MM-dd", CultureInfo.InvariantCulture);
                updatedFiles.Add(OracuMundial2026DataFiles.FifaRankingsCsv);
            ...
            for (var offset = 0; offset <= lookbackDays; offset++)
            {
                ...
                try
                {
                    ...
                    await WriteAtomicAsync(Path.Combine(dataDirectory, OracuMundial2026DataFiles.EloCsv), csv, ct);
                    eloRows = rows.Count;
                    eloDate = date;
                    updatedFiles.Add(OracuMundial2026DataFiles.EloCsv);
