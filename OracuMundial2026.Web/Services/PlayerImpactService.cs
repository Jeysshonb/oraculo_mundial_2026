...
namespace OracuMundial2026.Web.Services
{
    public class PlayerImpactService
    {
        ...
        public PlayerImpactService(HttpClient http, IWebHostEnvironment environment, IOptions<OracuMundial2026Config> config)
        {
            _http = http;
        ...
        {
            var path = Path.Combine(_environment.ContentRootPath, "Data", OracuMundial2026DataFiles.GoalscorersCsv);
            if (File.Exists(path))
