...
namespace OracuMundial2026.Web.Services
{
    public class CsvImportService
    {
        private readonly OracuMundial2026DbContext _db;
        private readonly IWebHostEnvironment _environment;

        public CsvImportService(OracuMundial2026DbContext db, IWebHostEnvironment env)
        {
            _db = db;
            _environment = env;
        }
        ...
        private async Task ImportGroupsAsync(CancellationToken ct)
        {
            ...
            var groupRows = CsvParsingHelper.ReadCsv<GroupCsvRow>(FullPath(OracuMundial2026DataFiles.GroupsCsv));
            ...
            foreach (var row in groupRows)
            {
                ...
                teams[id] = new Team { Id = id, Name = name, Source = OracuMundial2026DataFiles.GroupsCsv };
            }
            ...
            foreach (var group in groupRows.GroupBy(r => r.Group.Trim()).OrderBy(g => g.Key))
            {
                _db.Groups.Add(new Group
                {
                    ...
                    Source = OracuMundial2026DataFiles.GroupsCsv,
                });
            }
        }
        ...
        private async Task ImportRatingsAsync(CancellationToken ct)
        {
            ...
            var eloRows = CsvParsingHelper.ReadCsv<EloCsvRow>(FullPath(OracuMundial2026DataFiles.EloCsv));
            foreach (var row in eloRows)
            {
                if (!double.TryParse(row.Elo, NumberStyles.Float, CultureInfo.InvariantCulture, out var elo))
                    continue;

                await CreateTeamIfMissing(row.Team, OracuMundial2026DataFiles.EloCsv, ct);
                _db.Ratings.Add(new Rating
                {
                    ...
                    Source = OracuMundial2026DataFiles.EloCsv
                });
            }

            var fifaRows = CsvParsingHelper.ReadCsv<FifaCsvRow>(FullPath(OracuMundial2026DataFiles.FifaRankingsCsv));
            foreach (var row in fifaRows)
            {
                if (!double.TryParse(row.Points, NumberStyles.Float, CultureInfo.InvariantCulture, out var points))
                    continue;

                await CreateTeamIfMissing(row.Team, OracuMundial2026DataFiles.FifaRankingsCsv, ct);
                _db.Ratings.Add(new Rating
                {
                    TeamId = TeamNameNormalizer.ToId(row.Team),
                    ...
                    Value = points,
                    AsOf = DateTimeOffset.UtcNow,
                    Source = OracuMundial2026DataFiles.FifaRankingsCsv
                });
            }
        }
        ...
        private async Task ImportHistoricalResultsAsync(CancellationToken ct)
        {
            ...
            var rows = CsvParsingHelper.ReadCsv<HistoricalResultCsvRow>(FullPath(OracuMundial2026DataFiles.HistoricalResultsCsv));
            ...
            foreach (var row in rows)
            {
                ...
                await CreateTeamIfMissing(row.HomeTeam, OracuMundial2026DataFiles.HistoricalResultsCsv, ct);
                await CreateTeamIfMissing(row.AwayTeam, OracuMundial2026DataFiles.HistoricalResultsCsv, ct);
                ...
                _db.Results.Add(new MatchResult
                {
                    ...
                    Source = OracuMundial2026DataFiles.HistoricalResultsCsv
                });
            }
        }
                ...
                for (var i = 0; i < teams.Count; i++)
                {
                    for (var j = i + 1; j < teams.Count; j++)
                    {
                        _db.Fixtures.Add(new Fixture
                        {
                            ...
                            AwayTeamId = teams[j],
                            NeutralVenue = true,
                            Source = $"derivado de {OracuMundial2026DataFiles.GroupsCsv}"
                        });
                    }
                }
            }
