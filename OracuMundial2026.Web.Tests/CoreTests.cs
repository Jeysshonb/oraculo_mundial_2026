...
using OracuMundial2026.Web.Helpers;
using OracuMundial2026.Web.Models;
using OracuMundial2026.Web.Models.ApiFootballModels;
using OracuMundial2026.Web.Models.CsvModels;
using OracuMundial2026.Web.Predictors;
...
namespace OracuMundial2026.Web.Tests;
...
public class CoreTests
{
    ...
    public async Task RankingRefresh_WalksBackToLatestAvailableEloDateAndWritesParseableCsvs()
    {
        ...
        try
        {
            var options = Options.Create(new OracuMundial2026Config
            {
            ...
            Assert.Equal(2, CsvParsingHelper.ReadCsv<FifaCsvRow>(Path.Combine(root, "Data", OracuMundial2026DataFiles.FifaRankingsCsv)).Count);
            Assert.Equal(2, CsvParsingHelper.ReadCsv<EloCsvRow>(Path.Combine(root, "Data", OracuMundial2026DataFiles.EloCsv)).Count);
        }
        finally
        {
    ...
    public async Task RankingRefresh_DoesNotOverwriteExistingCsvsWhenSourcesCannotParse()
    {
        ...
        try
        {
            ...
            var fifaPath = Path.Combine(data, OracuMundial2026DataFiles.FifaRankingsCsv);
            var eloPath = Path.Combine(data, OracuMundial2026DataFiles.EloCsv);
    ...
    public async Task SnapshotService_SavesTournamentSnapshotAgainstLegacyNonNullProbabilityColumns()
    {
        ...
        var options = new DbContextOptionsBuilder<OracuMundial2026DbContext>().UseSqlite(connection).Options;
        await using var db = new OracuMundial2026DbContext(options);
    ...
    private static SimulationService Simulation(OracuMundial2026DbContext db, int simulations, int seed)
    {
    ...
    private static IOptions<OracuMundial2026Config> SimulationOptions(int simulations, int seed) =>
        Options.Create(new OracuMundial2026Config
        {
    ...
    private static IOptions<OracuMundial2026Config> AvailabilityOptions(string[] sources) =>
        Options.Create(new OracuMundial2026Config
        {
    ...
    private static ApiFootballService ApiService(OracuMundial2026DbContext db, HttpMessageHandler handler)
    {
        var options = Options.Create(new OracuMundial2026Config
        {
    ...
    private static async Task<OracuMundial2026DbContext> ImportedDb()
    {
        var db = await NewDb();
    ...
    private static async Task<OracuMundial2026DbContext> NewDb()
    {
        var connection = new SqliteConnection("Data Source=:memory:");
        await connection.OpenAsync();
        var options = new DbContextOptionsBuilder<OracuMundial2026DbContext>().UseSqlite(connection).Options;
        var db = new OracuMundial2026DbContext(options);
    ...
    private static string WebProjectRoot()
    {
        ...
        {
            var candidate = Path.Combine(current.FullName, "OracuMundial2026.Web");
            if (File.Exists(Path.Combine(candidate, "Data", OracuMundial2026DataFiles.GroupsCsv)))
                return candidate;

            current = current.Parent;
        }

        return Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "OracuMundial2026.Web"));
    }

    private static string NewTempRoot()
    {
        var root = Path.Combine(Path.GetTempPath(), "OracuMundial2026Tests", Guid.NewGuid().ToString("N"));
    ...
    {
        var rows = CsvParsingHelper.ReadCsv<HistoricalResultCsvRow>(Path.Combine(WebProjectRoot(), "Data", OracuMundial2026DataFiles.HistoricalResultsCsv));
    ...
    {
        public string ApplicationName { get; set; } = "OracuMundial2026.Web.Tests";
